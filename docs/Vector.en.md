...
| `public` | `explicit`                                                                     |        | `Vector(const T& value)`               | `const T& value`-Single value of type `T`                                                 | Initialize all vector elements with the single input value     |
| `public` | -                                                                              |        | `Vector(std::initializer_list<T>)`     | `std::initializer_list<T> InitializeList`                                                 | Initialize from list of `T`; if size != `dimension`, it errors |
| `public` | `template<typename T2, typename = std::enable_if_t<std::is_arithmetic_v<T2>>>` |        | `Vector(const Vector<T2, dimension>&)` | `const Vector<T2, dimension>& other`-another vector with diffrent type but same dimension | -                                                              |
...
| Access             | Modifier | Return                 | Name                                     | Parameter                              | Description                     |
| ------------------ | -------- | ---------------------- | ---------------------------------------- | -------------------------------------- | ------------------------------- |
| `private` `friend` | -        | `Vector<T, dimension>` | `operator-(const Vector<T, dimension>&)` | `const Vector<T, dimension>& vec`-向量 | Vector supports direct negation |
...
| -        | `template<typename T, size_t dimension>` | `std::ostream&` | `operator<<(std::ostream&, const Vector<T,dimension>&)` | `std::ostream& out`-output,<br>`const Vector<T,dimension>& vec`-vector to output | Output to console (this is a global function) |
